...
with tf.Graph().as_default():
    ...
    with tf.name_scope('fully-connected'):
        W_fc = weight_variable([int(32 * 200 * scale * FILTER_NUM4), FEATURE_DIM], name='weight_fc')
        b_fc = bias_variable([FEATURE_DIM], name='bias_fc')
        h_pool_flat = tf.reshape(h_pool4, [-1, int(32 * 200 * scale * FILTER_NUM4)])
    ...
    with tf.Session() as sess:
        train_writer = tf.train.SummaryWriter(SUMMARY_DIR + '/train2_2', sess.graph)
        ...
        for n in range(1, 8):
            ...
            data_manager.pre_process(False)
            data_manager.shuffle_data_and_events(REAL_IMAGE_HEIGHT)

            # saver
            saver = tf.train.Saver()

            ckpt = tf.train.get_checkpoint_state('./saver_cnn_sl/train2_2/')
            if ckpt:
            ...
            saver.save(sess, "./saver_cnn_sl/train2_2/model.ckpt")
            print('== accuracy ==')
