...
with tf.Graph().as_default():
    ...
    with tf.name_scope('fully-connected'):
        W_fc = weight_variable([int(8 * 400 * scale * FILTER_NUM2), FEATURE_DIM], name='weight_fc')
        b_fc = bias_variable([FEATURE_DIM], name='bias_fc')
        h_pool_flat = tf.reshape(h_norm2, [-1, int(8 * 400 * scale * FILTER_NUM2)])
    ...
    with tf.Session() as sess:
        train_writer = tf.train.SummaryWriter(SUMMARY_DIR + '/train2_4', sess.graph)
        ...
        for n in range(1, 8):
            ...
            data_manager.pre_process(True, [2, 3, 5, 6, 7, 10, 13]) # F7,F3,F4,F8,FC5,FC6,Cz
            data_manager.shuffle_data_and_events(REAL_IMAGE_HEIGHT)

            # saver
            saver = tf.train.Saver()

            ckpt = tf.train.get_checkpoint_state('./saver_cnn_sl/train2_4/')
            if ckpt:
            ...
            saver.save(sess, "./saver_cnn_sl/train2_4/model.ckpt")
            print('== accuracy ==')
