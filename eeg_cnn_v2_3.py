...
with tf.Graph().as_default():
    ...
    with tf.name_scope('fully-connected'):
        W_fc = weight_variable([int(8 * 200 * scale * FILTER_NUM2), FEATURE_DIM], name='weight_fc')
        b_fc = bias_variable([FEATURE_DIM], name='bias_fc')
        h_pool_flat = tf.reshape(h_norm2, [-1, int(8 * 200 * scale * FILTER_NUM2)])
    ...
    with tf.Session() as sess:
        train_writer = tf.train.SummaryWriter(SUMMARY_DIR + '/train2_3', sess.graph)
